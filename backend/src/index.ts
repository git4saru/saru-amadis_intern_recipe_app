...
import { FavouriteRecipes } from "./prisma-schema";
...
app.get("/api/recipes/favourite", async (req, res) => {
  try {
    ...
    const recipes: FavouriteRecipes[] = await prismaClient.favouriteRecipes.findMany();

    console.log(recipes);
    
    const recipeIds = recipes.map((recipe: FavouriteRecipes) => recipe.recipeId.toString());
